...
namespace DAL_QuanLyKhachSan
{
    public class DBUtil
    {
        private static string connString = @"Data Source=ADMIN-PC\SQLEXPRESS;Initial Catalog=Xuong_QuanLyKhachSan;Integrated Security=True;Encrypt=True;Trust Server Certificate=True";
        ...
        public static SqlDataReader Query(string sql, List<object> agrs, CommandType cmdType = CommandType.Text)
        {
            try
            {
                SqlCommand cmd = GetCommand(sql, agrs, cmdType);
            ...
            {
                throw new Exception("Lỗi khi thực hiện Query: " + ex.Message, ex);
            }
